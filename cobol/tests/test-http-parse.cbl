...
test-simple-get section.
    ...
    call "HTTP-PARSE" using
        ws-request-buffer ws-request-path ws-path-len
    end-call
...
test-long-path section.
    move spaces to ws-request-buffer
    move "POST /list/comments HTTP/1.1" to ws-request-buffer
    call "HTTP-PARSE" using
        ws-request-buffer ws-request-path ws-path-len
    end-call
    call "assert-equals" using "/list/comments",
        ws-request-path(1:14).
    move 14 to ws-expected-len
    call "assert-equals" using ws-expected-len, ws-path-len.
